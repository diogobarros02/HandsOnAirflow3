from typing import Any, ClassVar, Mapping, Callable, Sequence, Collection
...
class _SQLDecoratedOperator(DecoratedOperator, SQLExecuteQueryOperator):
    ...
    def __init__(self, *, python_callable= Callable, op_args: Collection[Any] | None = None, op_kwargs: Mapping[str, Any] | None = None,
                 **kwargs) -> None:
        
        if kwargs.pop("multiple_outputs", None):
            warnings.warn(
                f"`multiple_outputs` is not supported for {self.custom_operator_name} tasks. Ignoring.",
                UserWarning,
                stacklevel=3,
            )
        super().__init__(
    ...
    def execute(self, context: Context) -> Any:
        ...
        if not isinstance(self.sql, str) or self.sql.strip() == "":
            raise ValueError(f"The SQL returned by the function {self.python_callable} is invalid: {self.sql}")
...
def sql_task(python_callable: Callable | None = None,
